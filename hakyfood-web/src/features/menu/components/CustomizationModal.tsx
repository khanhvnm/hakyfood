    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-fade-in">
      <div className="relative bg-hk-bg-surface w-full max-w-lg rounded-3xl overflow-hidden shadow-2xl flex flex-col max-h-[90vh]">
        ...
        <div className="relative aspect-video w-full bg-hk-bg-surface-sunken">
          {(() => {
            const allMedia = [
              food.videoUrl ? { type: 'video', url: food.videoUrl } : null,
              food.imageUrl ? { type: 'image', url: food.imageUrl } : null,
              ...(food.detailImageUrls || []).map((url) => ({ type: 'image', url }))
              ...
              <>
                ...
                {allMedia.length > 1 && (
                  <>
                    <button
                      type="button"
                      onClick={() => setActiveImageIndex((prev) => (prev === 0 ? allMedia.length - 1 : prev - 1))}
                    ...
                    <button
                      type="button"
                      onClick={() => setActiveImageIndex((prev) => (prev === allMedia.length - 1 ? 0 : prev + 1))}
